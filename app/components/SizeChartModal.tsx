    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[32px] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 fade-in duration-300 flex flex-col max-h-[85vh] border border-gray-100">
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
          <div>
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Size Guide</h2>
            <p className="text-xs font-bold text-purple-600 uppercase tracking-widest mt-1">Footwear Collection</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="overflow-y-auto p-6 custom-scrollbar flex-1">
          <div className="space-y-8">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50">
                    <th className="px-4 py-3.5 text-xs font-black text-gray-400 uppercase tracking-widest">EU</th>
                    <th className="px-4 py-3.5 text-xs font-black text-gray-400 uppercase tracking-widest">US</th>
                    <th className="px-4 py-3.5 text-xs font-black text-gray-400 uppercase tracking-widest">UK</th>
                    <th className="px-4 py-3.5 text-xs font-black text-gray-400 uppercase tracking-widest">CM</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {SIZE_DATA.map((row, index) => (
                    <tr key={index} className="hover:bg-purple-50/30 transition-colors group">
                      ...
                      <td className="px-4 py-3 text-sm font-black text-gray-900 group-hover:text-purple-600 transition-colors">{row.cm}<span className="text-[10px] ml-0.5 text-gray-400">cm</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="p-6 bg-gradient-to-br from-purple-50/50 to-pink-50/50 rounded-[24px] border border-purple-100/50 relative overflow-hidden group">
              ...
              <h3 className="font-black text-gray-900 mb-4 flex items-center gap-2 relative z-10">
              ...
              <div className="space-y-4 relative z-10">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-white border border-purple-100 flex items-center justify-center text-[10px] font-black text-purple-600 shadow-sm">
                      {i + 1}
                    </div>
        ...
        <div className="p-6 bg-white border-t border-gray-100 sticky bottom-0 z-10">
          <button 
            onClick={onClose}
            className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all shadow-xl shadow-gray-200 active:scale-[0.98]"
          >
            I UNDERSTAND
          </button>
        </div>
