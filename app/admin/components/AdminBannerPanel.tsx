            ...
            <div className={`relative overflow-hidden bg-gray-100 rounded-xl border border-gray-300 mx-auto transition-all duration-300 ${previewMode === 'mobile' ? 'w-[375px] h-[667px] flex flex-col' : 'w-full flex'}`}>
              ...
              {previewMode === 'desktop' && currentBanner.textPosition !== 'OVERLAY' ? (
                <div className="grid grid-cols-2 w-full min-h-[300px]">
                  <div className={`relative ${currentBanner.textPosition === 'OUTSIDE_LEFT' ? 'order-2' : 'order-1'}`}>
                    {currentBanner.desktopImageUrl ? (
                      <Image src={currentBanner.desktopImageUrl} alt="Preview" fill className="object-cover" />
                    ) : (
                ...
                <div className={`relative w-full ${previewMode === 'desktop' ? 'aspect-[21/9]' : 'flex-1'}`}>
                  {currentBanner.desktopImageUrl && previewMode === 'desktop' && (
                    <Image src={currentBanner.desktopImageUrl} alt="Preview" fill className="object-cover" />
                  )}
                  {currentBanner.mobileImageUrl && previewMode === 'mobile' && (
                    <Image src={currentBanner.mobileImageUrl} alt="Preview" fill className="object-cover" />
                  )}
                  {!currentBanner.desktopImageUrl && previewMode === 'desktop' && (
                    <div className="absolute inset-0 flex items-center justify-center text-gray-400">Desktop Image Area</div>
                  )}
                  {!currentBanner.mobileImageUrl && previewMode === 'mobile' && (
                    <div className="absolute inset-0 flex items-center justify-center text-gray-400">Mobile Image Area</div>
                  )}
      ...
      <div className="space-y-10">
        ...
        {banners.length === 0 && (
          <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 mb-4">You haven't created any banners yet.</p>
            <button
              onClick={startNewBanner}
