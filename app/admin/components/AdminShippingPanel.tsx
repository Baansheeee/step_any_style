...
export default function AdminShippingPanel() {
  ...
  const fetchRegions = async () => {
    try {
      ...
      alert('Failed to load shipping regions');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddRegion = async () => {
    ...
    try {
      ...
      alert(`Failed to add region: ${error}`);
    }
  };
  ...
  const handleUpdateRegionCost = async (regionId: string) => {
    try {
      ...
      alert(`Failed to update region cost: ${error}`);
    }
  };
  ...
  const handleAddCity = async (regionId: string) => {
    ...
    try {
      ...
      alert(`Failed to add city: ${error}`);
    }
  };
  ...
  const handleDeleteRegion = async (regionId: string) => {
    ...
    try {
      ...
      alert(`Failed to delete region: ${error}`);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <p className="text-gray-600">Loading shipping regions...</p>
      </div>
    );
  }
      ...
      <div className="grid gap-4">
        {regions.length === 0 ? (
          <div className="text-center py-12 bg-slate-50 rounded-lg border-2 border-dashed border-slate-200">
            <p className="text-slate-500 italic">No shipping regions found. Add your first region to get started!</p>
          </div>
        ) : (
          regions.map((region) => (
